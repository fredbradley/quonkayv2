...
//Image Locations
$large_image_location = $upload_path.$_FILES['image']['name']; 		// New name of the large image
...
if (isset($_POST["photoupload"])) { 
	...
	if((!empty($_FILES["image"])) && ($_FILES['image']['error'] == 0)) {
		...
		$error= "Select a jpeg image, that is less than approx 5MB for upload. Your file was ".$_FILES['image']['size'];
		$error .= "<br /><pre>";
	...
	if (strlen($error)==0){
		...
		header("location: ../index.php?page=images&missioncomplete");
		exit();
	}
}
 //echo $error;
header("Location: ../index.php?page=images&message=AllGood");
